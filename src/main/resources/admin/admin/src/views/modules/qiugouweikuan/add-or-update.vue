	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else class="input" label="订单编号" prop="dingdanbianhao">
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="select" v-if="type!='info'"  label="尾款比例" prop="weikuanbili">
					<el-select :disabled="ro.weikuanbili" v-model="ruleForm.weikuanbili" placeholder="请选择尾款比例" >
						<el-option
							v-for="(item,index) in weikuanbiliOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else class="input" label="尾款比例" prop="weikuanbili">
					<el-input v-model="ruleForm.weikuanbili"
						placeholder="尾款比例" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="尾款" prop="weikuan">
					<el-input v-model="weikuan" placeholder="尾款" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.weikuan" label="尾款" prop="weikuan">
					<el-input v-model="ruleForm.weikuan" placeholder="尾款" readonly></el-input>
				</el-form-item>
...
export default {
	...
	computed: {
		...
		weikuan:{
			get: function () {
				return 1*this.ruleForm.shuliang*this.ruleForm.jiage*this.ruleForm.weikuanbili
			}
		},
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='weikuanbili'){
							this.ruleForm.weikuanbili = obj[o];
							this.ro.weikuanbili = true;
							continue;
						}
						if(o=='weikuan'){
							this.ruleForm.weikuan = obj[o];
							this.ro.weikuan = true;
							continue;
						}
						if(o=='guige'){
            ...
            this.weikuanbiliOptions = "0.8".split(',')
    ...
    // 提交
    onSubmit() {
        this.ruleForm.weikuan = this.weikuan
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `qiugouweikuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.qiugouweikuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
